...
# get balances
def get_mkr_balances(date):

    query = f"""
        SELECT address, raw_balance
        FROM maker.balances.mkr
        WHERE date = '{date}'
        """
...
def get_mkr_transfers(date):

    query = f"""
        SELECT timestamp, sender, receiver, raw_amount
        FROM MAKER.TRANSFERS.MKR
...
def processing(last_day, next_day):
    ...
    balances = get_mkr_balances(last_day)
    # get transfers from next day after last day available in balances table
    transfers = get_mkr_transfers(next_day)
...
def update_mkr_balances():
    ...
    if not last_day:
        last_day = datetime(2017, 11, 24).date()
