...
def _votes(operations, new_polls, **setup):
    ...
    try:
        ...
        old_polls = sf.execute(
            """select code, end_timestamp from staging_votes.internal.yays where type = 'poll'; """
        ).fetchall()
        for i in old_polls:
        ...
        for (
            ...
            # polls operations - only for new records
                ...
                if slate and option:
                    ...
                    # end_hot_wallet = balance_of(setup['mkr_address'], voter, 12902758)

                    # ... plus the current stake from Chiefs
                    if voter in voters:
                        for chief_address in setup['chief_addresses']:
                            initial_voting_power += voters[voter]['stakes'].get(chief_address, 0)
                    ...
                    # temp_record = [order_index, block, timestamp, tx_hash, to_address, voter, amount, slate, yay1, yay2, yay3, yay4, yay5, function, option, proxy, cold]
                    prev_choose = 0
                    for i in records:

                        if i[10] == poll and i[6] == voter and i[1] != order_index and i[4] != tx_hash:

                            prev_choose = dict()
                            prev_choose['voting_power'] = i[12]
                            prev_choose['option'] = i[11]

                        if i[1] == order_index and i[4] == tx_hash and i[6] == voter:
                            break
